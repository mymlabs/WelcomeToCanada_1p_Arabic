...
// 
	function init(){
		...
		titleScreenText = "";
		endScreenText = "شكرًا على قيامك باللعب! نود أن نشكر مجموعة جمعية الشبان المسيحيين في مانيتوبا للوافدين الجدد: عبد الرحمن، وأكرم، وعلاء، وعمار، وأسماء، وأسراء، وأونر، وبراء، ودلع، وجوانا، ومجد، ومايا، وميغيل، ومحمد، ونصوح، وأسامة، ووسام للمساعدة في تطوير هذا المورد.";
		...
		endURLs = [
			...
			["سبب الحياة - خط منع الانتحار والدعم في مانيتوبا","http://reasontolive.ca/",halfWidth,76],
			["NorWest Co-op Community Health","https://norwestcoop.ca/",halfWidth,143],
			["مركز أورورا للعلاج الأسري","http://www.aurorafamilytherapy.com/newcomer.html",halfWidth,197],
			["خدمات شباب ماكدونالد","https://www.mys.ca/services/youth-crisis-services",halfWidth,251],
			["منظمة مجتمع المهاجرين واللاجئين في مانيتوبا (IRCOM)","https://www.ircom.ca/",halfWidth,297],
			["New Journey Housing","https://www.newjourneyhousing.com/",halfWidth,364],
			["Klinic Community Health","http://klinic.mb.ca/",halfWidth,412],
			["خط مساعدة الأطفال","https://kidshelpphone.ca/",halfWidth,456],
		]		
	}//End of init
...
//            |__/      
	...
	function updateScreen(){
		...
		if(GAME_STATE === "loading"){
			...
			c.fillText("جارٍ التحميل...", halfWidth, halfHeight);
		}
		...
		if(GAME_STATE === "title"){
			...
			c.textAlign = "center";
			c.fillStyle = "#FFF";
			c.fillText("مرحبًا بكم في كندا",halfWidth,237);
		...
		if(GAME_STATE === "questions"){
			...
			if(showAnswerButtons === true && popUpActive === false){
				c.drawImage(answerA,352,179);
				c.drawImage(answerB,352,263);
				if(answer3Text != ""){
					c.drawImage(answerC,352,343);
					c.drawImage(answerD,352,422);
				}
			}
			...
			if(popUpActive === true && showPopUpText === true){
				...
				c.drawImage(didYouKnowHeader,172,28);
	...
	function changeState(){
		switch(GAME_STATE){
			...
			case "loading":
				...
				setTimeout(function(){
					titleScreenText = "مرحبًا بكم في كندا هي لعبة بسيطة تم إنشاؤها بالتعاون مع شباب الوافدين الجدد في وينيبيغ، مانيتوبا.  في هذه اللعبة، تعرّف على المال، والوظائف، والعثور على المساعدة، والعادات الاجتماعية الكندية، وكيفية التعامل مع التغيير، والتخطيط للمستقبل.",
			...
			case "title":
				...
				activeTitleObjects[activeTitleObjects.length] = new titleObject(pickCategory,-290,15,155,15);
			...
			case "categories":
				...
				setTimeout(function(){
					activeTitleObjects[activeTitleObjects.length] = new titleObject(answerA,-62,179,352,179);
					activeTitleObjects[activeTitleObjects.length] = new titleObject(answerB,-200,263,352,263);
					if(answer3Text != ""){
						activeTitleObjects[activeTitleObjects.length] = new titleObject(answerC,-300,343,352,343);
						activeTitleObjects[activeTitleObjects.length] = new titleObject(answerD,-400,422,352,422);
					}
			...
			case "questions":
				if(questionAnswered === false){
					...
					if(popUpText != "" && popUpActive === false){
						...
						setTimeout(function(){
							showPopUpText = true;
							activeTitleObjects[activeTitleObjects.length] = new titleObject(continueBtn,137,screenHeight,137,515);
						},500);
						showQuestionText = false;
						categorySelected = false;
					}else{
...
//    
	...
	function checkAnswer(playerGuess){
		...
		if(playerGuess === thisQuestion.correctAnswer){
			...
			activeTitlePopups[activeTitlePopups.length] = new titlePopUp(bigCheck,382,answerYPos);
			activeTitleObjects[activeTitleObjects.length] = new titleObject(correctText,screenWidth,10,290,10);
		}else{
			categoryScores[currentCategory-1].push(0);
			activeTitlePopups[activeTitlePopups.length] = new titlePopUp(bigX,382,guessYPos);
			activeTitlePopups[activeTitlePopups.length] = new titlePopUp(bigCheck,382,answerYPos);
			activeTitleObjects[activeTitleObjects.length] = new titleObject(sorryText,screenWidth,10,290,10);
		}
